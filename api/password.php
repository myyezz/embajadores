...
$query  = "update embajador set hashp='$hashp', nuevo='NO' where email='$email'";
...
$query = "select * from embajador where email='$email' and status='activo'";
$result = mysqli_query($link, $query);
if ($row = mysqli_fetch_array($result)) {
   if ($row["status"]=="activo") {
      ...
      if ($hashp==$row["hashp"]) {
         ...
         $quer2 = "select * from tienda where id=$idtienda and status='activo'";
