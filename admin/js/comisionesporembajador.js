const URL = 'https://myyezz.github.io/embajadores/api/comisionesporembajador.php';
const GRUPOS = 'https://myyezz.github.io/embajadores/api/listaembajadores.php';

let registros, registrosx;
...
const busqueda = () => {
   ...
   LISTA = URL + "?idembajador="+xembajador + "&desde="+xdesde + "&hasta="+xhasta;
   fetch(LISTA)
   .then((response) => response.json())
   .then((responseData) => {
      if (responseData.exito==="SI") {
...
const llenartabla = (registros) => {
   if (registros.length>0) {
      registros.forEach((elemento, indice) => {
         ...
         aprobacion = elemento.aprobacion.substr(8,2)+'/'+elemento.aprobacion.substr(5,2)+'/'+elemento.aprobacion.substr(0,4)
         txt5 = document.createTextNode(aprobacion);
         cl6 = document.createElement("td");
         cl6.appendChild(txt5);
